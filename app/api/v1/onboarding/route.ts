...
function convertDietaryPreferencesFromDB(preference: string): string {
  // Normalize to match the expected enum values
  ...
  const mapping: Record<string, string> = {
    ...
    'other': 'Other allergies/restrictions'
  };
...
function convertIdealWeekendFromDB(weekend: string): string {
  ...
  const mapping: Record<string, string> = {
    ...
    'adventure_exploration': 'Adventure and exploration',
    'relaxation and self-care': 'Relaxation and self-care',
    'relaxation_self_care': 'Relaxation and self-care',
    'social activities with friends': 'Social activities with friends',
    'social_activities': 'Social activities with friends',
    'cultural activities (museums, shows)': 'Cultural activities (museums, shows)',
    'cultural_activities': 'Cultural activities (museums, shows)',
    'sports and fitness': 'Sports and fitness',
    'sports_fitness': 'Sports and fitness',
    'home projects and hobbies': 'Home projects and hobbies',
    'home_projects': 'Home projects and hobbies',
    'mix of active and relaxing': 'Mix of active and relaxing',
    'mix_active_relaxing': 'Mix of active and relaxing'
  };
  ...
  if (normalized.includes('adventure') || normalized.includes('exploration')) {
    return 'Adventure and exploration';
  }
  if (normalized.includes('relaxation') || normalized.includes('relax') || normalized.includes('self-care')) {
    return 'Relaxation and self-care';
  }
  if (normalized.includes('social') || normalized.includes('friends')) {
    return 'Social activities with friends';
  }
  if (normalized.includes('cultural') || normalized.includes('museum') || normalized.includes('show')) {
    return 'Cultural activities (museums, shows)';
  }
  if (normalized.includes('sport') || normalized.includes('fitness') || normalized.includes('gym')) {
    return 'Sports and fitness';
  }
  if (normalized.includes('home') || normalized.includes('project') || normalized.includes('hobbies')) {
    return 'Home projects and hobbies';
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.step1.gender) {
      profileUpdateData.gender = toSnakeCase(validatedData.step1.gender);
    }
    if (validatedData.step1.city) {
      profileUpdateData.city = validatedData.step1.city;
    }
    if (validatedData.step1.nationality) {
      profileUpdateData.nationality = validatedData.step1.nationality;
    }
...
export async function GET() {
  try {
    ...
    const onboardingData = {
      ...
      step7: {
        dietaryPreferences: convertDietaryPreferencesFromDB(lifestyle?.dietary_restrictions?.[0] || 'No restrictions'),
      ...
      step8: {
        idealWeekend: convertIdealWeekendFromDB(socialPreferences?.ideal_weekend || 'Mix of active and relaxing'),
      },
    };
